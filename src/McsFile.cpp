// INTEL-HEX file (.mcs) interpreter class.
...
// N25Q128: 128Mb = 16MB = 0x1000000, page = 256B, sector = 64KB
//static const int MEM_SZ        = 0x1000000;
//static const int MEM_PAGE_SZ   = 0x100;
//static const int MEM_SECTOR_SZ = 0x10000;

// PC28F00BP30: 2Gb = 256MB = 0x10000000, page = 1024B, sector = 128KB
static const int MEM_SZ        = 0x10000000;
static const int MEM_PAGE_SZ   = 0x400;
static const int MEM_SECTOR_SZ = 0x20000;
...
unsigned char* McsFile::page( int page_no )
{
...
bool McsFile::pageIsEmpty( int page_no )
{
  if( page_no < MEM_SZ/MEM_PAGE_SZ )
    {
      bool empty = true;
      unsigned char *page = &_mem[page_no*MEM_PAGE_SZ];
      for( int i=0; i<256; ++i )
        if( page[i] != 0xFF )
          {
            empty = false;
...
int McsFile::sectorSize()
{
  return MEM_SECTOR_SZ;
}

// ----------------------------------------------------------------------------

int McsFile::memorySize()
{
  return MEM_SZ;
}
...
void McsFile::setFile( const std::string &filename )
{
  FILE         *fp;
  char          inp[100], *pch;
  int           line_no = 0;
  ...
  if( _minAddr >= MEM_SZ )
    {
      McsException exc( "file does *not* appear to contain HEX code " );
      throw exc;
    }
  if( _maxAddr >= MEM_SZ )
    {
...
void McsFile::interpretHexline( char *hexline,
...
unsigned char McsFile::str2Byt( char *str )
{
  unsigned char val = 0;
...
McsException::McsException( const std::string &str, int line_no, int data )
...
McsException::McsException( const std::string &str, int line_no )
...
McsException::McsException( const std::string &str )
